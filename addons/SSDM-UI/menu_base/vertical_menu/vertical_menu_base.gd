...
func _ready() -> void:
	...
	if not is_inside_tree():
		return
	expanded_height = panel_container.get_combined_minimum_size().y
...
func slide_open() -> void:
	...
	tween.tween_property(clip_wrapper, "custom_minimum_size:y", expanded_height, DURATION)
...
func slide_closed() -> void:
	...
	tween.set_ease(Tween.EASE_OUT)
	tween.set_trans(Tween.TRANS_CUBIC)
	tween.tween_property(clip_wrapper, "custom_minimum_size:y", 0.0, DURATION)
