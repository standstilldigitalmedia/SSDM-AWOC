...
func set_ref_uid(new_uid: int) -> AWOCResult:
	if new_uid == ResourceUID.INVALID_ID:
		return AWOCResult.failure("AWOCWork: Invalid resource reference UID: " + str(new_uid))
	res_uid = new_uid
...
func get_ref_path() -> AWOCResult:
	...
	if res_path.is_empty():
		return AWOCResult.failure("AWOCWork: Could not determine path for resource")
	return AWOCResult.success("", res_path)


func get_base_ref_path() -> AWOCResult:
	var ref_path_result: AWOCResult = get_ref_path()
	if ref_path_result.error:
		return ref_path_result
	var ref_path: String = ref_path_result.data
	return AWOCResult.success("", ref_path.get_base_dir())
...
func get_resource() -> Resource:
	...
	if not resource:
		push_error("AWOCWork: Failed to load resource at: " + res_path)
		return null
